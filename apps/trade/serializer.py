...
class ShopCartSerializer(serializers.Serializer):
    ...
    # 这里是继承Serialzer,必须制定queryset对象，如果继承ModelSerializer则不需要指定
    # goods是一个外键，可以用个这方法获取goods object中所有的值
...
class ShopCarDetailSerializer(serializers.ModelSerializer):
    # 购物车商品详情
    # 一个购物车对应一个商品
    goods=GoodsSerialzers(many=False,read_only=True)
    class Meta:
        model=ShoppingCart
        fields=('goods','nums')
...
class OrderGoodsSerializer(serializers.ModelSerializer):
    goods=GoodsSerialzers(many=False)
    class Meta:
        model=OrderGoods
        fields='__all__'
...
class OrderSerializer(serializers.ModelSerializer):
    ...
    oder_sn=serializers.CharField(read_only=True)
    pat_time=serializers.DateTimeField(read_only=True)
    ...
    def validate(self, attrs):
        # validate中添加order_sn，然后在view中既可以save
