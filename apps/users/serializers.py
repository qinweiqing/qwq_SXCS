...
from qwq_SXCS.settings import REGEX_MOBILE
...
class UserRegSerializer(serializers.ModelSerializer):
    # 用户注册
    # userprofile中没有code字段，这里需要自定义一个coede序列化字段
    ...
    def validate_code(self, code):
        # 用户注册，已post方式提交注册信息，post的数据都保存在initial_dat里面
        ...
        if verify_records:
            ...
            if five_mintes_ago>last_record.add_time:
                raise serializers.ValidationError('验证过期')
...
#         code是自己添加得，数据库中并咩有这个字段，验证完就删除掉
        del attrs['code']
        return attrs
    class Meta:
        model = User
